...
function createShape() {
    return {
        ...
        speed: Math.random() * 2 + 1 // Скорость движения фигуры
    };
}
...
function visualize() {
    ...
    //     // Изменяем цвет линии в зависимости от аудиоданных
    //     const lineHeight = dataArray[Math.floor(Math.random() * bufferLength)] / 2;
    //     line.width = lineHeight / 10; // Изменяем ширину линии в зависимости от громкости
    //     line.color = `rgba(${lineHeight}, ${255 - lineHeight}, ${lineHeight}, 0.5)`; // Изменяем цвет
    // });

    // Рисуем и обновляем фигуры
    shapes.forEach(shape => {
        ...
        // Двигаем фигуры
        shape.x += shape.speed; // Двигаем фигуру вправо
        if (shape.x > canvas.width) shape.x = 0; // Если выходит за границы, возвращаем в начало

        // Изменяем размер фигуры в зависимости от аудиоданных
        const shapeHeight = dataArray[Math.floor(Math.random() * bufferLength)] / 2;
        shape.size = shapeHeight / 5; // Изменяем размер фигуры в зависимости от громкости
        shape.color = `rgba(${shapeHeight}, ${255 - shapeHeight}, ${shapeHeight}, 0.5)`; // Изменяем цвет
    });
}
